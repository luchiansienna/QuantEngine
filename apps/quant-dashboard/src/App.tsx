    ...
    <main>
      ...
      <div className="workspace">
        <form className="panel controls" onSubmit={submit}>
          <div className="panel-heading"><h2>Instrument</h2><button type="button" className="text-button" onClick={() => { setInput(defaults); setUseCurve(false); setCurve(initialCurve); void run(defaults) }}>Reset</button></div>
          <Field label="Face value" hint="currency units" value={input.faceValue} min={.01} step="any" onChange={value => change('faceValue', value)} />
          ...
          <Field label="Maturity" hint="years" value={input.maturityYears} min={1 / input.paymentsPerYear} max={100} step="any" onChange={value => change('maturityYears', value)} />
          ...
          <label className="curve-toggle"><input type="checkbox" checked={useCurve} onChange={e => setUseCurve(e.target.checked)} /> Use a zero-rate curve</label>
          {useCurve ? <CurveEditor points={curve} onChange={setCurve} /> : <Field label="Market yield" hint="% annually" value={input.yield * 100} min={-50} max={100} step={.1} onChange={value => change('yield', value / 100)} />}
...
function Dashboard({ result }: { result: BondAnalysisResponse }) {
  ...
  const curved = result.valuationMode === "curve"
  return <>
    <section className="metrics">
      <Metric label="Present value" value={fmt.format(m.presentValue)} note="cash flows discounted today" />
      <Metric label={curved ? "Curve duration" : "Modified duration"} value={fmt.format(m.modifiedDuration)} note={curved ? "parallel zero-rate sensitivity" : "price sensitivity to yield"} />
      ...
      <Metric label={curved ? "PV-weighted timing" : "Macaulay duration"} value={`${fmt.format(m.macaulayDuration)}y`} note="weighted cash-flow timing" />
    </section>
    {curved && <CurveResults result={result} />}
    <Chart scenarios={result.scenarios} />
    <ScenarioTable scenarios={result.scenarios} curved={curved} />
