    ...
    <main>
      ...
      <div className="workspace">
        <form className="panel controls" onSubmit={submit}>
          ...
          <label className="field"><span>Coupon frequency <small>payments / year</small></span><select value={input.paymentsPerYear} onChange={event => change('paymentsPerYear', +event.target.value)}><option value="1">Annual</option><option value="2">Semi-annual</option><option value="4">Quarterly</option><option value="12">Monthly</option></select></label>
...
function Field({ label, hint, value, min, max, step, onChange }: FieldProps) {
  return <label className="field"><span>{label}<small>{hint}</small></span><input type="number" required value={value} min={min} max={max} step={step} onChange={event => onChange(event.target.valueAsNumber)} /></label>
}
